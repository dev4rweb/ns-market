...
//define("EDOST_ID", 4497); //Идентификатор магазина (вписать вместо: 0)
//define("EDOST_PSWD", "RReopLKzxMuqHWfxYrQCsT2ccJEvAeaF"); //Пароль для доступа к серверу расчетов (вписать вместо: password)
//---------- ВНИМАНИЕ!!! ----------
...
class Edost{
    ...
    var $site_win = true; //если 1, тогда перекодировать в WIN (utf8_win) - это результаты расчетов
    var $index, $curElem;
    var $status = 0;
    var $parser;
    var $rz;
    ...
    function edost_calc_post() {
        ...
        if (
            ...
            */
            ...
            //print"Город $to_city";
//$to_city="тульская областьп";


///Загружаем массив и делаем подмену
            require("massiv_sity_glob.php"); //подключение класса edost

//print"Берем из Сесии";

//print" <br>данные в классе $ind_dost  ,   $kray_dost ,  $city_dost ,  $fio_dost , $edost_weight_t , $edost_strah_t  <br>";

//$fio_dost=$_SESSION[edost_dost_mass][fio_dost];
//$email_dost=$_SESSION[edost_dost_mass][email_dost];
//$mob_phone_dost=$_SESSION[edost_dost_mass][mob_phone_dost];
            $ind_dost=$_SESSION[edost_dost_mass][edost_zip];
//$adress_dost=$_SESSION[edost_dost_mass][adress_dost];
            $kray_dost=$_SESSION[edost_dost_mass][to_region_n];
            $city_dost=$_SESSION[edost_dost_mass][to_city_n];
//$shd_dost=$_SESSION[edost_dost_mass][shd_dost];
//$OperTypes=$_SESSION[edost_dost_mass][OperTypes];
//$komment_zak=$_SESSION[edost_dost_mass][komment_zak];
            $edost_weight_t=$_SESSION[edost_dost_mass][edost_weight];
            $edost_strah_t=$_SESSION[edost_dost_mass][edost_strah];


//print" <br>данные в классе $ind_dost  ,   $kray_dost ,  $city_dost ,  $fio_dost , $edost_weight_t , $edost_strah_t  <br>";

            $to_city_n_tek=$_POST['to_city_n'];

            if(!$to_city_n_tek){
//print "В пост нет в сессии город  $city_dost и  $to_city_n_tek <br>";
                $to_city_n_tek=$city_dost;
            }

//print"i $to_city    ";

//print"Город 1  $to_city_n_tek";
//удалитм букву г. если есть
            $to_city_n_tek=trim(str_replace ( 'г.' , '' , $to_city_n_tek)); //Узнаем текущий артикул

//print"Город 111  $to_city_n_tek";

//print_r($massiv_glog_city );

            if($to_city_n_tek=='Орёл'){ $to_city_n_tek="Орел";             }

            if(in_array($to_city_n_tek,$massiv_glog_city  )){
//if(in_array($_SESSION[edost_dost_mass][to_city_n],$massiv_glog_city  )){
//print"Найден";
                $to_city=trim($to_city_n_tek);
///$to_city=trim($_SESSION[edost_dost_mass][to_city_n]);
//print"город-$to_city";
            }else{

                $to_city=trim($_POST['to_region_n']);
//print"По региону";
                if(!$to_city){$to_city=$kray_dost;
//print"НЕ Найден регион $kray_dost <br>";
                }

//$to_city=trim($_SESSION[edost_dost_mass][to_region_n]);
            }
...
//$to_city ="Тула";
            ...
            $to_city=str_replace ( 'обл.' , 'область' , $to_city ) ;

//print"Город 2  $to_city";


            if(!$weight){
                $weight=$edost_weight_t ;
//print"веса нет возьмем из сесии $edost_weight_t ";
            }

            $strah = preg_replace("/[^\d.]/","",preg_replace("/,/",".", substr($_POST['edost_strah'],0,12) )); //сумма для страховки
            if(!$strah){
                $strah=$edost_strah_t ;
//print"Страховки нет возмем из сесии $edost_strah_t ";
            }
            if ($strah=="") $strah = 0;
            ...
            if(!$zip or $zip==''){
                $zip=$ind_dost ;
//print"Индекса нет возмем из сесии $zip ";
            }
            ...
            $dop_ves_procent=$_SESSION[edost_dost_mass][massiv_dostavka][15];
            $weight =$weight + ($weight * $dop_ves_procent/100);
...
//            if( $kons_login==1075    ){  }
            ...
            if($to_city=="Башкортостан республика"){ $to_city="Республика Башкортостан";              }
            if($to_city=="Адыгея республика"){ $to_city="Республика Адыгея";              }

//print"Данные в класс  $to_city, $weight, $strah, $length, $width, $height, $zip, $from_city , sag_3_kart=$sag_3_kart  <br>      ";


            if(mb_substr_count($to_city,"Ханты-Мансийский Автономный округ -" )){ //Условие наличия в переменной строки prod_zak_
                $to_city2  =str_replace ( 'Ханты-Мансийский Автономный округ -' , '' , $to_city); //ПОлучаем строку которая начинается с артикла
//print" <br>TCNM $to_city2 <br>";
                $to_city    ="Ханты-Мансийский АО";
            }

//print"$to_city    ";


            if($to_city=='Орёл'){ $to_city="Орел";             }
